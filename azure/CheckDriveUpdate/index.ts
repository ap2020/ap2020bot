...
const main: AzureFunction = async (context: Context, timer: any, lastDate: { ts: number }): Promise<{ ts: number }> => ({ ts: (await checkUpdate(context, new Date(lastDate.ts))).getTime() });
