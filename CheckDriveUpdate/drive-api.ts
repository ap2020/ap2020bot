...
export const fetchDriveItem =
    cacheCalls(
        async (client: drive_v3.Drive, id: string): Promise<DriveItem> =>
            new DriveItem(client, (await client.files.get({
                fileId: id,
                fields: 'id,name,parents,mimeType,webViewLink,permissions',
            })).data),
        (c, id) => id,
    );
 ...
 * folderId => path
 */
const paths: Map<string, Promise<{path: string | null; valid: boolean}>> =
    new Map([[rootFolderId, Promise.resolve({ path: '/', valid: true })]]);
export const getPath = async (client: drive_v3.Drive, item: DriveItem): Promise<string> => {
     ...
     */
    const rec = cacheCalls(async (drive: drive_v3.Drive, folderId: string): Promise<{path: string | null; valid: boolean}> => {
        const folder = await fetchDriveItem(drive, folderId);
        ...
        const parentPaths = (
            (await Promise.all(folder.content.parents.map(async parentId => rec(drive, parentId))))
        ...
        if (parentPaths.length > 0) {
            ...
            return {
                path: `${parentPaths[0]}${folder.content.name}${isFolder ? '/' : ''}`,
                valid: true,
            };
        }
...
export const isIgnored = cacheCalls(
    async (client: drive_v3.Drive, itemId: string): Promise<boolean> => {
        ...
        const isParentsIgnored = await Promise.all(
            item.content.parents.map(async parentId => isIgnored(client, parentId)),
        );
        // 全ての親がignoredならtrue
    ...
    (c, id) => id,
    new Map([
        ...(process.env.GOOGLE_DRIVE_IGNORED_IDS ?? '')
            .split(',')
            .map(s => [s.trim(), Promise.resolve(true)] as const),
